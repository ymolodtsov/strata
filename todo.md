...
- [ ] **Formatting race condition** — styled text (bold, italic, code) flashes unstyled
      ...
      (moved to `makeNSView` only) but not yet confirmed stable in production use.
- [ ] **Attributed string fragility** — the entire inline formatting system (markdown-style
      ...
      initial setup can blow away the styled rendering.

## Recurring: Tab Bar Issues

- [ ] **"+" button in tab bar** — the automatic new-tab button appears in the tab bar even
      ...
      that also disables system tab merging features.
...
- [ ] **Multi-line selection discoverability** — Escape now enters selection mode (select
      current node), and Shift+Up/Down extends selection, but this isn't obvious to users.
      No visual affordance hints at this capability.
